...
|--------|----------------|
| Frontend | **Single static `index.html`**: vanilla JavaScript, no React/Next. |
...
| API key (Maps) | Set in-page as `window.STREET_WHISPERER_GMAPS_KEY` (browser-visible; must be restricted by HTTP referrer in Google Cloud). |
...
| Routing / deploy | **`vercel.json`**: rewrite non-`/api/*` paths to **`/index.html`** for SPA-style hosting. |
